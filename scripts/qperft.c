// Compile command: gcc -O3 -w -o perft qperft.c
// Source: https://home.hccnet.nl/h.g.muller/dwnldpage.html
...
char noUnder = 0, queen_dir[] = {1, -1, 16, -16, 15, -15, 17, -17}, king_rose[] = {1, 17, 16, 15, -1, -17, -16, -15},
...
unsigned long long int HashKey = 8729767686LL, HighKey = 1234567890LL, count, epcnt, xcnt, ckcnt, cascnt, promcnt,
                       nodecount; /* stats */
FILE* f;
clock_t ttt[30];
...
void move_gen(int color, int lastply, int d)
{
    ...
    { /* slider aimed at our king */
      ...
      { /* first on ray from King is ours */
        ...
        { /* our piece at x is pinned!  */
          ...
          else if (code[m] & capt_code[j - k] & C_DISTANT)
          { /* slider moves along pin ray */
            y = x;
            do
            { /* moves upto capt. pinner*/
              y += v;
              PUSH(z, y)
            } while (y != j);
            y = x;
  ...
  if (!(color & CasRights))
  {
    ...
    if (!(board[k - 1] ^ DUMMY | board[k - 2] ^ DUMMY | board[k - 3] ^ DUMMY | CasRights & color >> 2))
...
{ /* recursive perft, with in-lined make/unmake */
  ...
  int first_move, piece, victim, pred, succ, from, to, capt, mode, pcnr, vicnr, in_check = 0, checker = -1,
                                                                                check_dir = 20, legal;
      ...
      if (mode == 0xA1)
      {
        ...
        else if (--kind[piece - WHITE] >= KNIGHT)
        {
          ...
          { /* Knight must be put in Knight list */
            ...
            Zob[piece - WHITE] = Keys + 128 * KNIGHT + (color & BLACK) / 8 - 0x22;
          }
          else
            Zob[piece - WHITE] -= 128;
...
{ /* recursive perft, with in-lined make/unmake */
  ...
  for (i = flag ? ep1 : first_move; i < msp; i++) /* go through all moves */
  {
    ...
    else if (HashFlag)
    {
      ...
      if (depth > 2)
      {
        path[d] = stack[i];
        if (depth > 7)
        { // the count will not fit in 32 bits
          if (depth > 9)
          {
  ...
  minor:
    ...
    if ((piece != color && mode != 0xA0) || !capturable(color ^ COLOR, pos[color - WHITE]))
    {
  ...
  if (d <= Split && d > 1)
  {
    ...
    for (i = 1; i < d; i++)
    {
      printf("%c%c%c%c ", 'a' + (path[i] - 0x2222 >> 8 & 7), '1' + (path[i] - 0x2222 >> 12 & 7),
             'a' + (path[i] - 0x2222 & 7), '1' + (path[i] - 0x2222 >> 4 & 7));
      fprintf(f, "%c%c%c%c ", 'a' + (path[i] - 0x2222 >> 8 & 7), '1' + (path[i] - 0x2222 >> 12 & 7),
              'a' + (path[i] - 0x2222 & 7), '1' + (path[i] - 0x2222 >> 4 & 7));
    }
...
int main(int argc, char** argv)
{
  ...
  printf("Perft mode: ");
  if (HashFlag)
    printf("Hash-table size = %d%cB", (HashSize + 2) >> (HashSize < 64 * 1024 ? 6 : 16),
